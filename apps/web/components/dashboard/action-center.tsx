...
import { RecordHealthVisitDialog } from './record-health-visit-dialog';
...
export function ActionCenter() {
  ...
  const isFaculty = ['TEACHER'].includes(user?.role);
  const canModifyRegistry = isManagement;
  const canLogIncidents = isManagement || isFaculty;

  return (
    <Card className="border-none bg-white shadow-[0_8px_30px_rgb(0,0,0,0.02)] rounded-[2.5rem] overflow-hidden group">
      <CardHeader className="p-8 pb-4">
        <div className="flex items-center gap-3 mb-1">
            <div className="h-10 w-10 rounded-2xl bg-blue-600 flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 shadow-sm border border-blue-100">
      ...
      <CardContent className="p-8 pt-4 space-y-6">
        ...
        <div className="bg-slate-900 rounded-[2rem] p-6 shadow-2xl shadow-slate-900/20 relative overflow-hidden">
            ...
            <p className="text-xs font-medium text-slate-300 leading-relaxed italic mb-6 relative z-10">
                Automated institutional analysis is currently processing terminal sequences across all academic streams.
            </p>
